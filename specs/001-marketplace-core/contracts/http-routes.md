...
5. 성공 → 선점 확정 차감, 주문 `PAID` 생성, 판매자별 하위주문 분리+스냅샷 (FR-021/FR-023).
   실패 → 선점 해제, 주문 미생성, 실패 안내.
6. PRG로 완료 페이지 리다이렉트.

## 주문 조회·하위주문 (UC-10, UC-11, UC-12)

| 메서드 | 경로 | 인증 | 설명 |
|---|---|---|---|
| GET | `/orders` | 🛒 | 내 주문 목록·상태 (FR-016) |
| GET | `/orders/{id}` | 🛒 | 주문 상세(판매자별 하위주문·상태) (FR-022) |
| GET | `/seller/orders` | 🏪 | 내게 배정된 하위주문 목록(각 배송 상태 포함) (FR-010, 본인 것만) |
| POST | `/seller/orders/{subOrderId}/ship` | 🏪 | 해당 SubOrder의 Delivery `PENDING→SHIPPING` 전이 (FR-011) |
| POST | `/seller/orders/{subOrderId}/deliver` | 🏪 | Delivery `SHIPPING→DELIVERED` 전이(완료 시각 기록) |

- 배송 상태 전이는 소유 판매자만, 허용 전이만(그 외 400/403) (R4/FR-009). 상태는 배송
  컨텍스트의 Delivery가 보유(subOrderId로 연결).
