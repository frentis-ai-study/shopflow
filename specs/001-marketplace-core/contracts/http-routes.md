...
| GET | `/seller/orders` | 🏪 | 내게 배정된 하위주문 목록 (FR-010, 본인 것만) |
| POST | `/seller/orders/{subOrderId}/ship` | 🏪 | `PAID→SHIPPING` 전이 (FR-011) |
| POST | `/seller/orders/{subOrderId}/deliver` | 🏪 | `SHIPPING→DELIVERED` 전이(완료 시각 기록) |

- 상태 전이는 소유 판매자만, 허용 전이만(그 외 400/403) (R4/FR-009).
